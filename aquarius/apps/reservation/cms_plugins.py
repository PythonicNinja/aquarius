    ...
    def render(self, context, instance, placeholder):
        ...
        if form.is_valid():
            ...
            send_mail(u'Rezerwacja ze strony http://gdynia-magnolia.pl',
                      u'Osoba kontaktowa: %s,\nRezerwacja od: %s,\nRezerwacja do: %s,\nAdres e-mail: %s,\nTel.: %s,\nWłaściciel karty: %s, \nData ważności: %s, \nNr kart: %s, \nNr weryfikacyjny: %s' % (obj.last_name, obj.start_date, obj.end_date, obj.client_email_address, obj.phone, obj.name_on_card, obj.expiry_date, obj.card_number, obj.card_code ), 'Willa Magnolia <willa@gdynia-magnolia.pl>', ['willa@gdynia-magnolia.pl'])
            if obj.client_email_address:
                send_mail(u'Wiadomość ze strony gdynia-magnolia.pl potwierdzenie rezerwacji', u'Dziękujemy za rezerwację. Odpowiemy tak szybko jak to możliwe.', 'Willa Magnolia <willa@gdynia-magnolia.pl>', [obj.client_email_address])
            return HttpResponseRedirect('/dziekujemy/') # Redirect after POST
